...
`)}};function i(e){return typeof e==`object`&&e&&`str`in e?e.str:``}function a(e){return typeof e==`object`&&!!e&&`hasEOL`in e&&!!e.hasEOL}async function o(e){let{GlobalWorkerOptions:n,getDocument:r}=await t(async()=>{let{GlobalWorkerOptions:e,getDocument:t}=await import(`./pdfjs-vendor-bSpZQg3j.js`).then(e=>e.t);return{GlobalWorkerOptions:e,getDocument:t}},__vite__mapDeps([0,1]));n.workerPort=new Worker(new URL(`/pdf-extractor/assets/pdf.worker-BedBhH-j.js`,``+import.meta.url),{type:`module`});let o=await r({data:new Uint8Array(await e.arrayBuffer())}).promise,s=[];for(let e=1;e<=o.numPages;e+=1){let t=await(await o.getPage(e)).getTextContent(),n=[],r=``;for(let e of t.items){let t=i(e);if(t.length>0&&(r+=t),a(e)){let e=r.trim();e.length>0&&n.push(e),r=``}else t.length>0&&(r+=` `)}let c=r.trim();c.length>0&&n.push(c);let l=n.join(`
`);s.push({pageNumber:e,rawLines:n,rawText:l,parsedLines:n,parsedText:l})}return s}async function s(e,t){let n={documents:[],failures:[]};for(let r of e)try{let e=await o(r);n.documents.push({fileName:r.name,pageCount:e.length,pages:e.map(e=>({...e,parsedLines:e.rawLines.map((n,i)=>t.parseLine(n,{fileName:r.name,pageNumber:e.pageNumber,lineNumber:i+1})).filter(e=>e.length>0),parsedText:e.rawLines.map((n,i)=>t.parseLine(n,{fileName:r.name,pageNumber:e.pageNumber,lineNumber:i+1})).filter(e=>e.length>0).join(`
